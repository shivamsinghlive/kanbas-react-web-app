	...
	$(function () {
		...
		const main_nav = {
			...
			_init: function () {
				...
				main_nav.navlinks = $('header.header > .container > .navlinks--container > .navlinks');
				main_nav.submenuToggles = $(main_nav.navlinks).find('.menu-item-has-children > button.navlinks-showsubmenu');
				main_nav.parentMenuItems = $(main_nav.navlinks).find('.menu-item-has-children');
				...
				main_nav.searchToggle.on('click', '.nu__sitesearch_toggle', main_nav._did_click_searchform_toggle);
			...
			_reset_all_revealed_elems: function () {
				...
				if (window.innerWidth >= 1101) {
					main_nav.navlinks.parent('.navlinks--container').removeClass('revealed');
			...
			_did_click_submenu_toggle: function (e) {
				...
				let the_top_level_parent_item = $(e.target).closest('.navlinks > .menu > .menu-item');

				// hide any open menus that are not in this items lineage
				$(the_top_level_parent_item).siblings().removeClass('revealed');
				$(the_top_level_parent_item).siblings().find('.revealed').removeClass('revealed');
